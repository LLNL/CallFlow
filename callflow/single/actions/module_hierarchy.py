...
class moduleHierarchy:
        ...
        for idx, node in enumerate(nodes):
            ...
            if attr == "time (inc)":
                ...
                if entry_func_df.empty:
                    ret[node] = json.dumps({"name": "", "time": [], "time (inc)": []})
                else:
                ...
                ret[node] = self.df.loc[self.df["name"] == corrected_function][
                    "nid"
                ].tolist()
    ...
    def add_node_attributes(self):
        ...
        imbalance_perc_mapping = self.generic_map(
            self.hierarchy.nodes(), "imbalance_perc"
        )
        nx.set_node_attributes(
            self.hierarchy, name="imbalance_perc", values=imbalance_perc_mapping
        )

        component_path_mapping = self.generic_map(
            self.hierarchy.nodes(), "component_path"
        )
        nx.set_node_attributes(
            self.hierarchy, name="component_path", values=component_path_mapping
        )
    ...
    def run(self):
        ...
        for idx, node in enumerate(self.hierarchy.nodes()):
            if "/" in node:
                ...
                component_paths_df = self.df.loc[self.df["name"] == node][
                    "component_path"
                ].unique()

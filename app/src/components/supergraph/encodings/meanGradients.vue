...
export default {
	...
	methods: {
		...
		visualize() {
			...
			rectangles
				...
				.attrs({
					...
					"stroke-width": d => d.type == "intermediate" ? this.intermediateStrokeWidth : this.strokeWidth,
					"fill": d => this.fill_with_gradients(d, this.$store.selectedMetric, this.$store.distributionColor),
				});
		},

		stroke_by_metric(d) {
			let metric = this.$store.selectedMetric == "Inclusive" ? "time (inc)": "time";
		...
		fill_with_gradients(d, metric, color) {
			...
			const grid = this.$store.modules["ensemble"][index][metric]["gradients"]["hist"]["x"];
			const val = this.$store.modules["ensemble"][index][metric]["gradients"]["hist"]["y"];

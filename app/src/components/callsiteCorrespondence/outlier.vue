...
export default {
	...
	methods: {
		...
		ensembleOutliers() {
			...
			let data = this.$store.data_cs["ensemble"][this.callsite.name][this.$store.selectedMetric]["boxplots"]["outliers"];
		...
		targetOutliers() {
			...
			let callsite_data = this.$store.data_cs[this.$store.selectedTargetDataset][this.callsite.name];
			let data = [];

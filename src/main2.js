...
var calcLM3 = require('./graphBuilder.js');


var sankeySplitNode = require('./xmlParser2.js');
...
var nodeSplit = require('./nodeSplit.js');
...
nodeMetricReader.on('end', function(){
	...
	var xml2 = new sankeySplitNode(xmlTree, xmlFile, nodeMetric, sankeySplitNodeCallBack, configFile);	
...
function sankeySplitNodeCallBack(data){
	...
	sankeyDat["edges"].forEach(function(edge){
		var tempObj = JSON.parse(JSON.stringify(edge));
		edgesDeepCopy.push(tempObj);
	});
	nodesDeepCopy = JSON.parse(JSON.stringify( sankeyDat["nodes"] ));
	nodeListDeepCopy = JSON.parse(JSON.stringify( sankeyDat["nodeList"] ));
	edgeListDeepCopy = JSON.parse(JSON.stringify( sankeyDat["edgeList"] ));
	connInfoStatic = JSON.parse(JSON.stringify( sankeyDat["connInfo"] ));
	adjMatrixStatic = JSON.parse(JSON.stringify( sankeyDat["adjMatrix"] ));
	staticGraph = {"nodes" : nodesDeepCopy, "edges" : edgesDeepCopy, "nodeList" : nodeListDeepCopy, "edgeList" : edgeListDeepCopy, "connInfo" : connInfoStatic}
...
app.get('/getSankey', function(req, res){
	...
	Object.keys(nodesDeepCopy).forEach(function(lab){
		console.log(lab);
	})

	procIDArray = [];
	splitByParentList = [];
	adjMatrix = JSON.parse(JSON.stringify( adjMatrixStatic));
	connInfo = JSON.parse(JSON.stringify( connInfoStatic));
...
app.get('/splitNode', function(req, res){
	...
	var functionNames = {};
	idList.forEach(function(fid){
		var funcID = parseInt(fid);
		functionNames[funcID] = procedureTable[funcID];
	})

	var split = {
		"option" : 0,
		"sankeyLabel" : req.query["lmID"],
		"functions" : idList,
	...
	// JSON.parse(JSON.stringify(sankeyData["nodes"]));
	// JSON.parse(JSON.stringify(sankeyData["edges"]));
	// JSON.parse(JSON.stringify(sankeyData["nodeList"]));
	// JSON.parse(JSON.stringify(sankeyData["edgeList"]));
	// JSON.parse(JSON.stringify(adjMatrix));
	// JSON.parse(JSON.stringify(nodeInfo));
	// JSON.parse(JSON.stringify(connInfo));
	// JSON.parse(JSON.stringify(split));

	var splitting = new nodeSplit(sankeyData["nodes"], sankeyData["edges"], sankeyData["nodeList"], sankeyData["edgeList"], adjMatrix, nodeInfo, connInfo, split);
	var result = splitting.nodeSplit();

	sankeyData = result["graph"];	
	adjMatrix = sankeyData["adjMatrix"];
	connInfo = sankeyData["connInfo"];
	...
	console.log("there are,", sankeyData["edges"].length);
...
app.get('/getLists', function(req, res){
	...
	// if(staticGraph["nodes"][specialID] != null){
	if(sankeyData["nodes"][specialID] != null){
		var functionListResult = {};
		// var uniqueNodeID = staticGraph["nodes"][specialID]["uniqueNodeID"];
		var uniqueNodeID = sankeyData["nodes"][specialID]["uniqueNodeID"];
		uniqueNodeID.forEach(function(nodeID){
			// console.log(nodeID, nodeInfo);
			var procedureID = nodeInfo[nodeID]["procID"];
			if(functionListResult[procedureID] == null){
				functionListResult[procedureID] = {
					'procID' : procedureID,
					'name' : procedureTable[procedureID],

...
export default function Sankey() {
    ...
    function computeNodeBreadths() {
        ...
        // if(level < 5){
        //    widthScale = d3.scalePow().domain([level,level + 1]).range([0, size[0]]);
        // }
    
        widthScale = d3.scalePow().domain([0,level]).range([0, size[0]])//.exponent(.1);
        // moveSinksRight(level);
    ...
    function scaleNodeBreadths(kx) {
        nodes.forEach(function (node) {
            ...
            node.x = widthScale(level);
            console.log(node.name, node.level, node.x)
        });
    }
            ...
            nodesByBreadth.forEach(function (nodes) {
                nodes.forEach(function (node, i) {
                    ...
                    links.forEach(function (edge) {
                        if (edge["target"]['name'][0] == node['name'][0]) {
                            console.log(node.name)
                            if (edge["source"] != null && edge["source"]['y'] != null) {
                                console.log(edge['source']['height'])
                                nodeHeight = Math.max(nodeHeight, edge["target"]['y']);
                            }
                        }
                    });
                    node.maxY = nodeHeight
                    node.y = Math.min(node.maxY, i)

        ...
        init(graph) {
            ...
            if(this.$store.selectedAlignmentShow == 'All'){
                for(let i = 0; i < this.$store.datasets.length; i += 1){
                    let dataset = this.$store.datasets[i]
                    this.initEdges(dataset)
                }
            }
            else{
                this.initEdges(this.$store.selectedDataset)
            }
            ...
            if(this.$store.selectedDiffNodeAlignment == 'Middle'){
                ...
                if(this.$store.selectedAlignmentShow == 'All'){
                    for(let i = 0; i < this.$store.datasets.length; i += 1){
                        let dataset = this.$store.datasets[i]
                        this.drawTopEdges(dataset)
                    }
                }
                else if(this.$store.selectedAlignmentShow == 'Single'){
                    this.drawTopEdges(this.$store.selectedDataset)
                }
            }
        ...
        drawMiddleEdges(dataset) {
            this.edges.selectAll('.diff-edge-' + dataset)
                ...
                .attr('d', (d) => {
                    ...
                    d.sy = d.source.value * (source_unit) * (1 - d.source.props[dataset]) / 2
                    ...
                    let Bx0 = d.source.x + d.source.dx,
                        ...
                        Bx2 = Bxi(0.4),
                        Bx3 = Bxi(1 - 0.4),
                        By0 = d.source.y + this.$parent.ySpacing + d.sy + d._sy - d.__sy,
            ...
            this.edges.selectAll('.diff-edge-' + dataset)
                ...
                .attr('d', (d) => {
                    ...
                    let Tx0 = d.source.x + d.source.dx,
                        ...
                        Tx2 = Txi(0.4),
                        Tx3 = Txi(1 - 0.4),
                        Ty0 = d.source.y + this.$parent.ySpacing,
                    ...
                    d.sy = -d.weight * (d.source.height / d.source.value) * (1 - d.source.props[dataset])
                    d.ty = -d.weight * (d.target.height / d.target.value) * (1 - d.target.props[dataset])

                    if (ratio > 1) {
                        d.sy =  d.weight * (d.target.height/d.target.value)*d.target.props[dataset]
                    }
                    ...
                    if(ratio <= 1){
                        ...
                        By0 = d.source.y + d.sy + this.$parent.ySpacing
                        By1 = d.target.y + d.ty + d.target.height + this.$parent.ySpacing
                    }

...
function LMView(args) {
 ...
 top: 10, right: 30, bottom: 10, left: 10
    ...
    function calcStat() {
        ...
        Object.keys(visData).forEach((rowid) => {
    ...
    function calcCusterSize() {
        let rowID = Object.keys(visData);
        let sum = 0;
        visData[rowID[0]].forEach((dat) => {
	    var temp = {
    ...
    function render() {
        ...
        let colorScale = d3.scale.quantize().domain([minVal, maxVal]).range(['#f7fbff', '#deebf7', '#c6dbef', '#9ecae1', '#6baed6', '#4292c6', '#2171b5', '#084594']);
        // var colorScale = d3.scale.log().domain([minVal + 0.000001, maxVal]).range(['#deebf7','#084594']);

        let svgO = d3.select(containerID).append('svg')
        ...
        let containerRect = svgO.append('rect')
	    .attr('width', containerWidth)
	    .attr('height', containerHeight)
	    .style('fill', 'none')
        ...
        Object.keys(visData).forEach((rowid, idx) => {
        ...
        let matrixWidth = numbOfClusters * rectHeight;
        let unitWidth = matrixWidth / totalProcesses;
        ...
        let rowidList = Object.keys(visData);
        // var rowidList = ["5579"]
        ...
        // legend
        let lCols = ['#f7fbff', '#deebf7', '#c6dbef', '#9ecae1', '#6baed6', '#4292c6', '#2171b5', '#084594'];
        let legendG = svgBase.append('g').attr('opacity', 1)
	    .attr('transform', `translate( 100, ${  rectHeight * Object.keys(visData).length + 0 + 100  } )`);
        let legend = legendG.append('defs')
...
.attr('y2', '100%')
        ...
        let lRect = legendG.append('rect').attr('height', 20).attr('width', rectWidth * numbOfClusters).style('fill', 'url(#gradient)')
.attr('transform', 'translate(0,10)');

        let lScale = d3.scale.quantize().domain([minVal, (maxVal + minVal) / 2, maxVal])
        ...
        let yAxisLine = legendG.append('g').attr('class', 'y axis').attr('transform', 'translate(0,40)').call(yAxis);
        yAxisLine.append('text').attr('x', (rectHeight * numbOfClusters) / 2 + 20).attr('y', 10).attr('dy', '.71em')
...
.text('Inclusive Time');
    ...
    function drawArc(startLM, endLM, direction) {
        ...
        let tLM = Object.keys(visData);
        // console.log(tLM);

        let startLMCoord = []; // (x,y) of start lm

        // get index of start lm
        let startLMIndex = tLM.indexOf(startLM);
        let startLMY = (rectHeight * startLMIndex) + (rectHeight / 2);

        startLMCoord.push(xLocation);
        startLMCoord.push(startLMY);

        let endLMCoord = [];
        let endtLMIndex = tLM.indexOf(endLM);
        let endLMY = (rectHeight * endtLMIndex) + (rectHeight / 2);
    ...
    function calcArchPath(startPoint, endPoint, direction) {
        let arcRadius = (Object.keys(visData).length * rectHeight) / 2 * direction;
        let x0 = startPoint[0],
	    y0 = startPoint[1],
	    x1 = endPoint[0],
	    y1 = endPoint[1],

...
class FilterNetworkX:
    ...
    def filter_time_inc_overall(self, perc):
        log.info(f"Filtering by {perc}% inclusive runtime")
        df = self.df.loc[(self.df["time (inc)"] > perc * 0.01 * self.df['time (inc)'].max())]
        filter_call_sites = df["name"].unique()
        log.debug(f"Filtered callgraph node count: {len(filter_call_sites)}")
        return df[df["name"].isin(filter_call_sites)]

    def filter_time_exc_overall(self, perc):
        df = self.df.loc[(self.df["time"] > perc * 0.01 * self.df['time'].max())]
        filter_call_sites = df["name"].unique()
        log.debug(f"Number of nodes left in dataframe: {len(filter_call_sites)}", len(filter_call_sites))
        return df[df["name"].isin(filter_call_sites)]

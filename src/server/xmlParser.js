...
const fileLoader = function (xmlTree, xmlFile, callBack, configFileName, procIDArray, nodeMetrics, splitByParentList, nodeKeep) {
    ...
    parseProfileTable(obj.root().child(1).child(0));
    ...
    // Calculate Load module 
    // 1. loadModule
    // 2. maximum value ( Not sure of its use ) 
    // 3. LmIDToSplitConfig - Find the files and functions to split by from the config file 
    function parseLoadModule(xmlLoadModNode) {
        xmlLoadModNode.childNodes().forEach((loadMod) => {
            ...
            // Not sure why maxLMID is calculated 
    ...
    // Calculate metrics as runtimeID array
    // runTimeID = [{ n: name, t: time  }] 
    ...
    // Assign the root node manually. ( May be because cct is from child(1) 
    // EntryExit data:
    //       {    name: 'root',
    ...
    //       }
    // Sankey call information goes into three objects indexed by levels:
    //         NodeArray:{
    //            name: 'Root',
    //            inTime: 0,
    //            exTime: 0,
                ...
                if (fileName.indexOf(prefix) > 0) {
                    if (prefix.length > prefixLen) {
                        ...
                        if (splitLoadModuleFileID[node.loadModuleID][loadModName] == null) {
                            maxLMID += 1;
                            splitLoadModuleFileID[node.loadModuleID][loadModName] = {
                                prefixID,
                                fileList: [],
                                newID: maxLMID,
                            };
                ...
                functionNames.forEach((fName) => {
                    if (procedureTable[node.procedureID].includes(fName)) {
                        if (splitLoadModuleFileID[node.loadModuleID][functionToCheck] == null) {
                            maxLMID += 1;
                            splitLoadModuleFileID[node.loadModuleID][functionToCheck] = {
                                prefixID,
                                fileList: [],
                                newID: maxLMID,
                            };
    ...
    function addInfo(level, specialID, node, parentNode) {
        ...
        sanKeyMetricDataLM[level][specialID].push({
            nodeID: node.nodeID,
            parentLMID: parentNode.loadModuleID,
            parentNodeID: parentNode.nodeID,
            nodeLevel: level,
            parentProcID: parentNode.procedureID,

...
class Pipeline:
	...
	def convertdfToNetworkX(self, state, graph_type='entire'):
		convert = HatchetToNetworkX(state, graph_type=graph_type, path_column_name='path', construct_graph=True, add_data=False)
	...
	def filterNetworkX(self, states, state_name, perc):
		...
		state.graph = states[state_name].graph
